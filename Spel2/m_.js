...
function drawMenu() {
  ...
  if(menu == 4 || menu == 5) {
    ...
    for(var i = 0; i < levels; i++) {
      if(menu == 4) {
        ctx.fillStyle = "rgb(0, 0, 0, 0.75)";
        ctx.fillText("Level " + i + ": " + Math.round(highscores[i].normal) / 1000, 0.31 * c.width, (0.1 + 0.05 * i) * c.height);
      } else if(menu == 5) {
        ctx.fillStyle = "rgb(0, 0, 0, 0.75)";
        ctx.fillText("Level " + i + ": " + Math.round(highscores[i].challenge) / 1000, 0.31 * c.width, (0.1 + 0.05 * i) * c.height);
      }
    }
  }
